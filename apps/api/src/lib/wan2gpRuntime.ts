...
function buildWanScript(input: Wan2GpGenerateInput): string {
  ...
  return [
    ...
    "    return ('minimax' in token) or (' h3 ' in f' {token} ') or ('fl2va' in token) or ('ref2va' in token)",
    "",
    "def _is_h3_low_vram(value: str) -> bool:",
    "    token = _norm_token(value)",
    "    return _is_h3(value) and any(mark in token for mark in ['pruned', '20b', 'w4a8', 'int8', 'nvfp4', 'q4', 'gguf'])",
    "",
    "def _score_video_model(value: str, profile_hint: int) -> int:",
    "    token = _norm_token(value)",
    "    score = 100",
    "    if _is_h3(value):",
    "        score += 420 if profile_hint <= 3 else (320 if _is_h3_low_vram(value) else 160)",
    "    if 'ltx' in token:",
    "        score += 260 if profile_hint <= 2 else 150",
    "    if any(mark in token for mark in ['hunyuan', 'animate', 'longcat', 'kandinsky', 'magihuman']):",
    "        score += 210 if profile_hint <= 3 else 150",
    "    if any(mark in token for mark in ['t2v 1 3b', 't2v_1.3b', 'fun inp 1 3b', 'fun_inp_1.3b']):",
    "        score += 280 if profile_hint >= 4 else 170",
    "    if any(mark in token for mark in [' sf ', 'schnell', 'lightning']):",
    "        score += 220 if profile_hint >= 4 else 120",
    "    if any(mark in token for mark in ['1 3b', 'tiny', 'small']):",
    "        score += 120 if profile_hint >= 4 else 50",
    "    if any(mark in token for mark in ['22b', '33b', 'xl', 'hq']):",
    "        score += 80 if profile_hint <= 2 else -30",
    ...
    "else:",
    "    if mode == 'image':",
    "        for candidate in preferred_image_models:",
    "            if candidate in installed_model_types:",
    "                model_type = candidate",
    "                break",
    "    else:",
    "        if installed_model_types:",
    "            sorted_candidates = sorted(installed_model_types, key=lambda item: _score_video_model(item, profile), reverse=True)",
    "            model_type = sorted_candidates[0] if sorted_candidates else ''",
