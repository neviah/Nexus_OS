...
const UNITY_ACTION_NAMES: readonly UnityToolActionName[] = [
  "unity_compile",
  "unity_run_tests",
  "unity_get_logs",
  "unity_screenshot",
...
const DEFAULT_UNITY_ALLOWED_ACTIONS: UnityToolActionName[] = [
  "unity_compile",
  "unity_run_tests",
  "unity_get_logs",
  "unity_screenshot",
...
const DEFAULT_UNITY_MAX_ACTIONS_PER_TURN = 2;
const DEFAULT_UNITY_MAX_DYNAMIC_CODE_CHARS = 2000;

type UnityExecutionPolicy = {
  enabled: boolean;
...
function getUnityExecutionPolicy(state: SystemState): UnityExecutionPolicy {
  const rawAllowed = Array.isArray(state.toolPermissions?.unityAllowedActions)
    ? state.toolPermissions?.unityAllowedActions
    : DEFAULT_UNITY_ALLOWED_ACTIONS;
  const allowedActions = Array.from(new Set(rawAllowed.filter((entry): entry is UnityToolActionName => UNITY_ACTION_NAMES.includes(entry as UnityToolActionName))));
  const normalizedAllowed = allowedActions.length > 0 ? allowedActions : DEFAULT_UNITY_ALLOWED_ACTIONS;

  const rawAllowlist = Array.isArray(state.toolPermissions?.unityHarnessAllowlist)
    ? state.toolPermissions?.unityHarnessAllowlist
    : [];
  const harnessAllowlist = Array.from(new Set(rawAllowlist
    .filter((entry): entry is string => typeof entry === "string")
    .map((entry) => entry.trim())
    .filter((entry) => entry.length > 0)));

  const maxActionsRaw = Number(state.toolPermissions?.unityMaxActionsPerTurn ?? DEFAULT_UNITY_MAX_ACTIONS_PER_TURN);
  const maxActionsPerTurn = Number.isFinite(maxActionsRaw)
    ? Math.min(5, Math.max(1, Math.floor(maxActionsRaw)))
    : DEFAULT_UNITY_MAX_ACTIONS_PER_TURN;

  const maxCodeCharsRaw = Number(state.toolPermissions?.unityMaxDynamicCodeChars ?? DEFAULT_UNITY_MAX_DYNAMIC_CODE_CHARS);
  const maxDynamicCodeChars = Number.isFinite(maxCodeCharsRaw)
...
function setUnityExecutionPolicy(state: SystemState, input: Partial<UnityExecutionPolicy>): UnityExecutionPolicy {
  const current = getUnityExecutionPolicy(state);
  const nextAllowed = Array.isArray(input.allowedActions)
    ? Array.from(new Set(input.allowedActions.filter((entry) => UNITY_ACTION_NAMES.includes(entry))))
    : current.allowedActions;
  const nextAllowlist = Array.isArray(input.harnessAllowlist)
    ? Array.from(new Set(input.harnessAllowlist.map((entry) => entry.trim()).filter((entry) => entry.length > 0)))
    : current.harnessAllowlist;

  const maxActionsRaw = Number(input.maxActionsPerTurn ?? current.maxActionsPerTurn);
  const maxDynamicCodeRaw = Number(input.maxDynamicCodeChars ?? current.maxDynamicCodeChars);

  state.toolPermissions = {
    ...(state.toolPermissions ?? {}),
    unityExecutionEnabled: input.enabled ?? current.enabled,
    unityAllowedActions: (nextAllowed.length > 0 ? nextAllowed : DEFAULT_UNITY_ALLOWED_ACTIONS),
    unityHarnessAllowlist: nextAllowlist,
...
function validateUnityActionAgainstPolicy(input: {
  action: UnityToolAction;
  policy: UnityExecutionPolicy;
  source: "harness" | "api";
  harnessId?: string;
  ...
  if (!policy.enabled) {
    return { ok: false, reason: "Unity tool execution is disabled." };
  }

  if (!policy.allowedActions.includes(action.action)) {
    return { ok: false, reason: `Action ${action.action} is blocked by Unity policy.` };
  }

  if (input.source === "harness" && policy.harnessAllowlist.length > 0) {
    const harnessId = (input.harnessId ?? "").trim();
    if (!harnessId || !policy.harnessAllowlist.includes(harnessId)) {
      return { ok: false, reason: `Harness ${harnessId || "unknown"} is not in the Unity allowlist.` };
    }
  }

  if (action.action === "unity_compile" || action.action === "unity_run_tests" || action.action === "unity_get_logs") {
    return { ok: true, args: undefined };
  }

  if (action.action === "unity_screenshot") {
    const rawWindowName = typeof action.args?.windowName === "string"
      ? action.args.windowName.trim()
      : "Game";
    const windowName = rawWindowName.length > 0 ? rawWindowName : "Game";
    if (windowName.length > 80 || !/^[A-Za-z0-9 _().-]+$/.test(windowName)) {
      return { ok: false, reason: "unity_screenshot arguments are invalid. windowName must match ^[A-Za-z0-9 _().-]+$ and be <= 80 chars." };
    }
    return { ok: true, args: { windowName } };
  }

  const code = typeof action.args?.code === "string" ? action.args.code : "";
  if (!policy.allowDynamicCode) {
    return { ok: false, reason: "unity_execute_dynamic_code is disabled by policy." };
  }
  if (!code.trim()) {
    return { ok: false, reason: "unity_execute_dynamic_code requires non-empty args.code." };
  }
  if (code.length > policy.maxDynamicCodeChars) {
    return { ok: false, reason: `Dynamic code exceeds max allowed length (${policy.maxDynamicCodeChars}).` };
  }

  const bannedPatterns = [
    /\bSystem\.IO\b/i,
    /\bSystem\.Diagnostics\b/i,
    /\bSystem\.Net\b/i,
    /\bSystem\.Reflection\b/i,
    /\bSystem\.Runtime\.InteropServices\b/i,
    /\bDllImport\b/i,
    /\bFile\s*\./i,
    /\bDirectory\s*\./i,
    /\bProcess\s*\./i,
    /\bEnvironment\s*\.\s*(Exit|SetEnvironmentVariable)\b/i,
  ];
  if (bannedPatterns.some((pattern) => pattern.test(code))) {
    return { ok: false, reason: "Dynamic code contains blocked APIs (IO/process/network/reflection/interops)." };
  }
...
function getGameCreatorExecutionJobs(state: SystemState): GameCreatorExecutionJob[] {
  return state.gameCreator?.executionJobs ?? [];
}

function writeGameCreatorExecutionJobs(state: SystemState, jobs: GameCreatorExecutionJob[]): void {
  state.gameCreator = {
    ...(state.gameCreator ?? {}),
    executionJobs: jobs.slice(0, 100),
  };
}
...
function buildGameCreatorExecutionQueueFromStatus(status: Array<{
  ...
  const draftItems: GameCreatorQueueItem[] = status
    .filter((entry) => {
      ...
      if (mode === "auto-produce") {
        return entry.quality?.passed ?? true;
      }
      return entry.record.reviewStatus === "approved" && entry.record.locked;
    })
    .map((entry) => {
...
function buildGameCreatorScaffoldFiles(input: {
  ...
  const setup = spec.setupWizard;
  const root = "GameBuild";
  const stamp = new Date().toISOString();
  const files: Array<{ relativePath: string; content: string }> = [];

  files.push({
    relativePath: `${root}/PRODUCTION_NOTES.md`,
    content: [
      "# Production Notes",
      "",
      `Generated at ${stamp}`,
      `Target: ${setup.target}`,
      `Genre: ${setup.genre}`,
      `Scope Tier: ${setup.scopeTier}`,
      `Queue Task: ${item.title}`,
      `Lane: ${item.lane}`,
    ].join("\n"),
  });

  if (setup.target === "web-2d") {
    files.push(
      {
        relativePath: `${root}/web/package.json`,
        content: JSON.stringify({
          name: "nexus-generated-prototype",
          private: true,
          type: "module",
          scripts: {
            dev: "vite",
      ...
      {
        relativePath: `${root}/web/src/main.ts`,
        content: [
      ...
      {
        relativePath: `${root}/web/src/game.ts`,
        content: [
          "type GameConfig = {",
          "  seed: number;",
          "  width: number;",
          ...
          "};",
          "",
          "type Entity = { x: number; y: number; vx: number; vy: number; r: number; hp: number };",
          "",
          ...
          `  title.textContent = 'Playable ${setup.genre} Prototype (${setup.perspective})';`,
          "  const subtitle = document.createElement('small');",
          "  subtitle.textContent = 'WASD move, survive enemy swarms. Deterministic fixed-step simulation.';",
          "",
          ...
          "  canvas.style.border = '1px solid #28425f';",
          "  canvas.style.background = 'linear-gradient(#0f172a, #1e293b)';",
          "",
          "  const hud = document.createElement('small');",
          "  hud.textContent = 'HP 10 | Score 0';",
          "  root.append(title, subtitle, canvas, hud);",
          "",
          ...
          "  }",
          "",
          "  const rng = makeRng(GAME_CONFIG.seed);",
          "  const keys = new Set<string>();",
          "  const player: Entity = { x: canvas.width * 0.5, y: canvas.height * 0.5, vx: 0, vy: 0, r: 12, hp: 10 };",
          ...
          "    let x = margin;",
          "    let y = margin;",
          "    if (side === 0) { x = margin; y = rng() * canvas.height; }",
          "    if (side === 1) { x = canvas.width - margin; y = rng() * canvas.height; }",
          "    if (side === 2) { x = rng() * canvas.width; y = margin; }",
          "    if (side === 3) { x = rng() * canvas.width; y = canvas.height - margin; }",
          "    enemies.push({ x, y, vx: 0, vy: 0, r: 10, hp: 1 });",
          "  };",
          "",
          ...
          "    player.vx = (moveX / mag) * GAME_CONFIG.playerSpeed;",
          "    player.vy = (moveY / mag) * GAME_CONFIG.playerSpeed;",
          ...
          "",
          "    spawnTimer += dt;",
          "    if (spawnTimer >= GAME_CONFIG.spawnSeconds) {",
          "      spawnTimer = 0;",
          "      spawnEnemy();",
          "    }",
          "",
          ...
          "      enemy.vx = (dx / distance) * GAME_CONFIG.enemySpeed;",
          "      enemy.vy = (dy / distance) * GAME_CONFIG.enemySpeed;",
          "      enemy.x += enemy.vx * dt;",
          "      enemy.y += enemy.vy * dt;",
          "",
          ...
          "    }",
          "",
          "    hud.textContent = `HP ${Math.max(0, player.hp)} | Score ${score} | Enemies ${enemies.length}`;",
          "  };",
          "",
          ...
          "    accumulator += delta;",
          "",
          "    while (accumulator >= fixedStep && player.hp > 0) {",
          "      tick(fixedStep);",
      ...
      {
        relativePath: `${root}/web/src/simCore.ts`,
        content: [
          "export type SimState = { hp: number; score: number; enemies: number };",
          "",
          "export function runDeterministicSimulation(seed: number, ticks: number): SimState {",
          "  let state = seed | 0;",
          "  const rand = () => {",
          "    state = (1664525 * state + 1013904223) | 0;",
          "    return ((state >>> 0) % 100000) / 100000;",
          ...
          "      enemies += 1;",
          "    }",
          "    if (enemies > 0 && rand() > 0.98) {",
          "      enemies -= 1;",
          "      score += 3;",
          "    }",
          "    if (enemies > 0 && rand() > 0.995) {",
          "      hp -= 1;",
          "    }",
          "    if (hp <= 0) {",
          "      return { hp: 0, score, enemies };",
          "    }",
          "  }",
          "",
      ...
      {
        relativePath: `${root}/web/scripts/smoke-playtest.mjs`,
        content: [
          "function runDeterministicSimulation(seed, ticks) {",
          "  let state = seed | 0;",
          "  const rand = () => {",
          "    state = (1664525 * state + 1013904223) | 0;",
          "    return ((state >>> 0) % 100000) / 100000;",
          ...
          "  let enemies = 0;",
          "  for (let tick = 0; tick < ticks; tick += 1) {",
          "    if (tick % 120 === 0) enemies += 1;",
          "    if (enemies > 0 && rand() > 0.98) { enemies -= 1; score += 3; }",
          "    if (enemies > 0 && rand() > 0.995) hp -= 1;",
          "    if (hp <= 0) return { hp: 0, score, enemies };",
          "  }",
          "  return { hp, score, enemies };",
          "}",
          "",
          "const a = runDeterministicSimulation(1337, 900);",
          "const b = runDeterministicSimulation(1337, 900);",
          "if (JSON.stringify(a) !== JSON.stringify(b)) {",
          "  throw new Error(`Determinism failed: ${JSON.stringify(a)} vs ${JSON.stringify(b)}`);",
          "}",
          "if (a.score < 3) {",
          "  throw new Error(`Smoke playtest failed: score too low (${a.score}).`);",
          "}",
          "console.log('Smoke playtest passed.', a);",
        ].join("\n"),
      },
      {
      ...
      },
      {
        ...
        content: [
          ...
          "  throw new Error(`Missing asset manifest at ${manifestPath}`);",
          "}",
          "const manifest = JSON.parse(fs.readFileSync(manifestPath, 'utf-8'));",
          "if (!Array.isArray(manifest.sprites) || manifest.sprites.length === 0) {",
          "  throw new Error('Asset manifest must include at least one sprite entry.');",
          "}",
          "if (!Array.isArray(manifest.audio?.music) || !Array.isArray(manifest.audio?.sfx)) {",
          "  throw new Error('Asset manifest audio block is invalid.');",
          "}",
          "for (const entry of manifest.sprites) {",
          "  if (!entry.id || !entry.file || typeof entry.frameWidth !== 'number') {",
          "    throw new Error(`Invalid sprite entry: ${JSON.stringify(entry)}`);",
          "  }",
          "}",
      ...
      },
      {
        ...
        content: JSON.stringify({
          generatedAt: stamp,
          style: setup.artStyle,
          sprites: [
            { id: "hero_run", file: "sprites/hero_run.png", frameWidth: 64, frameHeight: 64, frames: 6 },
            { id: "enemy_slime", file: "sprites/enemy_slime.png", frameWidth: 64, frameHeight: 64, frames: 4 },
          ],
          audio: {
            music: ["audio/menu_theme.wav", "audio/combat_loop.wav"],
            sfx: ["audio/jump.wav", "audio/hit.wav", "audio/ui_confirm.wav"],
          },
        }, null, 2),
      },
      {
        relativePath: `${root}/web/assets/sprites/atlas.meta.json`,
        content: JSON.stringify({
          generatedAt: stamp,
          packing: {
            algorithm: "grid",
          ...
          atlases: [
            { id: "characters", output: "sprites/atlas_characters.png", sources: ["hero_run", "enemy_slime"] },
          ],
        }, null, 2),
      },
      {
        relativePath: `${root}/web/README.md`,
        content: [
          "# Generated Playable Prototype",
          "",
          "## Quick Start",
          "1. npm install",
          ...
          "4. npm run dev",
          "",
          "This build includes deterministic simulation checks and an asset-manifest pipeline baseline.",
        ].join("\n"),
      },
      {
        relativePath: `${root}/web/index.html`,
        content: [
          "<!doctype html>",
          "<html>",
          "  <head>",
          ...
          "    <title>NexusOS Generated Prototype</title>",
          "  </head>",
          "  <body>",
          "    <div id=\"app\"></div>",
    ...
    files.push(
      ...
      },
      {
        ...
        content: [
          ...
          "    private void Start()",
          "    {",
          `        Debug.Log(\"NexusOS bootstrap: ${setup.genre} / ${setup.perspective} / ${setup.target}\");`,
          "    }",
          "}",
        ].join("\n"),
      },
      {
        relativePath: `${root}/unity/Assets/Scripts/PlayerController.cs`,
        content: [
          "using UnityEngine;",
          "",
  ...
  if (item.lane === "art") {
    files.push({
      relativePath: `${root}/Assets/art/ART_TASKS.md`,
      content: [
        "# Art Tasks",
        "",
        "- Produce hero concept silhouettes",
        "- Produce 3 enemy family concept sheets",
        "- Approve palette and mood board",
      ].join("\n"),
    });
  }
  ...
  if (item.lane === "audio") {
    files.push({
      relativePath: `${root}/Assets/audio/sfx_manifest.json`,
      content: JSON.stringify({
        generatedAt: stamp,
        requiredSfx: ["jump", "attack", "hurt", "pickup", "ui_confirm"],
        musicCues: ["menu_theme", "combat_loop", "victory_sting"],
      }, null, 2),
    });
  }

  if (item.lane === "engineering") {
    files.push({
      relativePath: `${root}/GameSystems/DETERMINISTIC_SYSTEMS.md`,
      content: [
        "# Deterministic Systems Checklist",
        "",
        "- Fixed update timestep (60Hz) with accumulator",
        "- Seeded RNG for spawns and procedural events",
        "- Replay-safe event ordering",
        "- Data-first tuning in JSON assets",
      ].join("\n"),
    });
  }
  ...
  if (item.lane === "qa") {
    files.push({
      relativePath: `${root}/QA/SMOKE_TESTS.md`,
      content: [
        "# Smoke Tests",
        "",
        "- Run deterministic simulation smoke: npm --prefix web run smoke",
        "- Validate asset manifest integrity: npm --prefix web run validate:assets",
        "- Confirm game loop reaches score > 0 within 15 simulated seconds",
      ].join("\n"),
    });
  }

  if (item.lane === "art" || item.lane === "content" || item.lane === "production") {
    files.push({
      relativePath: `${root}/Assets/pipeline/PIPELINE_RULES.md`,
      content: [
        "# Asset Pipeline Rules",
        "",
        "- Every generated sprite/audio asset must be listed in web/assets/manifest.json",
        "- Atlas metadata updates are required when adding sprite sets",
        "- File naming convention: lowercase-with-underscores",
        "- Reject artifacts that fail technical validation or style consistency",
      ].join("\n"),
    });
  }
  ...
  if (item.lane === "design") {
    files.push({
      relativePath: `${root}/Assets/ui/UI_EXAMPLES.md`,
      content: [
        "# UI Example Targets",
        "",
        "- HUD wireframe with health, objective, and cooldown slots",
        "- Pause menu with controls and accessibility tab",
        "- Failure/retry screen with progression summary",
      ].join("\n"),
    });
  }
...
async function executeGameCreatorQueueItem(input: {
  ...
  if (!input.item.regenerateArtifact && input.spec.setupWizard.target === "web-2d") {
    ...
    outputs.push(reportRelativePath);
    createArtifact({
      kind: "code",
...
async function evaluateGate2Readiness(input: {
  ...
  for (const entry of status) {
    ...
    if (input.requireApproved !== false) {
    ...
    } else if (entry.record.reviewStatus === "approved") {
      approvedDocs += 1;
    }
...
async function executeUnityToolActions(input: {
  ...
  const outputs: string[] = [];
  const selectedActions = input.actions.slice(0, policy.maxActionsPerTurn);
  if (input.actions.length > selectedActions.length) {
...
function collectUnityToolActions(value: unknown, unity: UnityToolAction[]): void {
  ...
  if (UNITY_ACTION_NAMES.includes(normalized as UnityToolActionName)) {
    const args = typeof obj.arguments === "object" && obj.arguments !== null
      ? obj.arguments as Record<string, unknown>
      : (typeof obj.arguments === "string"
        ? tryParseMaybeJson(obj.arguments)
...
app.post("/api/tools/game-creator/process/start", async (req, res) => {
  ...
  const readiness = await evaluateGate2Readiness({
    state,
    workspacePath: workspace.path,
    requireLocked: requestedMode === "strict-approval" ? body.requireLocked !== false : false,
...
app.get("/api/tools/game-creator/execution", async (req, res) => {
  const workspaceId = String(req.query.workspaceId ?? "").trim();
  const state = await readSystemState();
  const workspace = await resolveWorkspaceContext(state, workspaceId || state.activeWorkspaceId);
  const mode = getGameCreatorExecutionMode(state);
...
app.post("/api/tools/game-creator/execution/mode", async (req, res) => {
...
app.post("/api/tools/unity/policy", async (req, res) => {
  const body = req.body as {
    enabled?: boolean;
    allowedActions?: UnityToolActionName[];
    allowDynamicCode?: boolean;
  ...
  const policy = setUnityExecutionPolicy(state, {
    ...
    allowDynamicCode: body.allowDynamicCode,
    harnessAllowlist: Array.isArray(body.harnessAllowlist) ? body.harnessAllowlist : undefined,
    maxActionsPerTurn: typeof body.maxActionsPerTurn === "number" ? body.maxActionsPerTurn : undefined,
